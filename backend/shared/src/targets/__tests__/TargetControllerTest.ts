...
describe('TargetControllerTest', () => {
  describe('getTargetsPerQuarter', () => {
    it('Should throw if quarter is greater than 4 ', () => {
      ...
      expect(controller.getTargetsPerQuarter(5, 2022)).rejects.toThrowError(trpc.TRPCError)
    })

    it('Should throw if quarter is less than 1 ', () => {
      const controller = container.resolve(TargetController)
      expect(controller.getTargetsPerQuarter(0, 2022)).rejects.toThrowError(trpc.TRPCError)
    })
  ...
  describe('getTargetsPerMonthForYear', () => {
    it('Should throw if month is less than 1 ', async () => {
      const controller = container.resolve(TargetController)
      await expect(controller.getTargetsPerMonthForYear(0, 2022)).rejects.toThrowError(trpc.TRPCError)
    })

    it('Should throw if month is greater than 12', async () => {
      const controller = container.resolve(TargetController)
      await expect(controller.getTargetsPerMonthForYear(13, 2022)).rejects.toThrowError(trpc.TRPCError)
    })
  ...
  describe('getTeamTargetsPerQuarter', () => {
    it('Should throw if quarter is less than 1 ', async () => {
      const controller = container.resolve(TargetController)
      await expect(controller.getTeamTargetsPerQuarter(0, 2022)).rejects.toThrowError(trpc.TRPCError)
    })

    it('Should throw if quarter is greater than 4', async () => {
      const controller = container.resolve(TargetController)
      await expect(controller.getTeamTargetsPerQuarter(5, 2022)).rejects.toThrowError(trpc.TRPCError)
    })
  ...
  describe('getTeamTargetsPerMonthForYear', () => {
    it('Should throw if month is less than 1 ', async () => {
      const controller = container.resolve(TargetController)
      await expect(controller.getTeamTargetsPerMonthForYear(0, 2022)).rejects.toThrowError(trpc.TRPCError)
    })
